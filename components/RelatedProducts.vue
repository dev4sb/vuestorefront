...
export default {
  ...
  setup() {
   const { addItem: addItemToCart, isInCart, cart: currentCart } = useCart();
    ...
    return { productGetters ,addItemToCart,isInCart,currentCart, sendNotification};
  },
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  data () {
    return {
  ...
  methods: {
    ...
    HandleAddTocart(productObj) {
      ...
      this.addItemToCart(productObj).then(() => {
        console.log('productObj',productObj.product.name)
        this.sendNotification({
          key: 'added_to_cart',
          message: `${productObj.product.name}has been successfully added to cart !`,
          type: 'success',
          title: 'Product added!',
          icon: 'check'
        });
